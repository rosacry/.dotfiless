...
def construct_waltz():
    waveObjects = []
    for i in range(16):
        waveObjects.append(simpleaudio.WaveObject.from_wave_file(minuet_filename(roll_dice(2))))
        waveObjects.append(simpleaudio.WaveObject.from_wave_file(trio_filename(roll_dice(1))))
    for objects in waveObjects:
        play = objects.play()  
        play.wait_done()
    return -1
